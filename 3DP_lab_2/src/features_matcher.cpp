...
{
    std::vector<cv::DMatch> raw_matches, inlier_matches;
    ...
    for (int i = 0; i < images_names_.size() - 1; i++)
    {
        for (int j = i + 1; j < images_names_.size(); j++)
        {
            ...
            // Get keypoints and descriptors for images i and j
            std::vector<cv::KeyPoint> &keypoints_i = features_[i];
            std::vector<cv::KeyPoint> &keypoints_j = features_[j];
            cv::Mat &descriptors_i = descriptors_[i];
            cv::Mat &descriptors_j = descriptors_[j];

            // Match descriptors
            cv::BFMatcher matcher(cv::NORM_L2);
            matcher.match(descriptors_i, descriptors_j, raw_matches);

            // Extract matched keypoints
            for (const cv::DMatch &match : raw_matches)
            {
                points_i.push_back(keypoints_i[match.queryIdx].pt);
                points_j.push_back(keypoints_j[match.trainIdx].pt);
            }

            // Estimate Essential matrix and get inlier matches
            cv::Mat mask;
            cv::Mat essential_matrix = cv::findEssentialMat(points_i, points_j, new_intrinsics_matrix_, cv::RANSAC, 0.999, 1.0, mask);
            ...
            for (size_t k = 0; k < mask.rows; ++k)
            {
                ...
                {
                    inlier_matches.push_back(raw_matches[k]);
                }
            }

            // Check if inlier matches are enough
            if (inlier_matches.size() <= 6)
            {
                std::cout << "Not enough inliers for image " << i << " and image " << j << ", skipping matching" << std::endl;
                continue;
            }

            // Estimate Homography matrix and get inlier matches
            cv::Mat homography_matrix = cv::findHomography(points_i, points_j, cv::RANSAC, 1.0, mask);
