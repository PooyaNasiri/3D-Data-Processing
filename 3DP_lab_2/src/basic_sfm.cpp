...
struct ReprojectionError
{
    ...
    bool operator()(const T *const camera, const T *const point, T *residuals) const
    {
        ...
        const T *const camera_rotation = camera;
        const T *const camera_translation = camera + 3;

        // Project 3D point into image plane
        T projected[3];
        ceres::AngleAxisRotatePoint(camera_rotation, point, projected);
        projected[0] += camera_translation[0];
        projected[1] += camera_translation[1];
        projected[2] += camera_translation[2];

        // Normalize
        T normalized_x = projected[0] / projected[2];
        T normalized_y = projected[1] / projected[2];

        // Compute residuals
        residuals[0] = normalized_x - T(observed_x);
        residuals[1] = normalized_y - T(observed_y);
...
bool BasicSfM::incrementalReconstruction(int seed_pair_idx0, int seed_pair_idx1)
{
    ...
    cv::Mat E = cv::findEssentialMat(points0, points1, intrinsics_matrix, cv::RANSAC, 0.001, 1.0, inlier_mask_E);

    // Estimate Homography matrix
    cv::Mat H = cv::findHomography(points0, points1, cv::RANSAC, 0.001, inlier_mask_H);

    // Check if the number of inliers for E is higher than for H
    ...
    // Check if the motion is mainly sideward
    if (fabs(t.at<double>(2)) > fabs(t.at<double>(0)) || fabs(t.at<double>(2)) > fabs(t.at<double>(1)))
    {
        return false;
    }

    // Store transformation
    R.convertTo(init_r_mat, CV_64F);
    t.convertTo(init_t_vec, CV_64F);
    ...
    for (int iter = 1; iter < num_cam_poses_ - 1; iter++)
    {
            ...
            if (cam_pose_optim_iter_[cam_idx] > 0)
            {
                for (auto const &co_iter : cam_observation_[cam_idx])
                {
                        ...
                        cam_observation_[new_cam_pose_idx].find(pt_idx) != cam_observation_[new_cam_pose_idx].end())
                    {
                        ...
                        /////////////////////////////////////////////////////////////////////////////////////////
                        // Extract 6D pose blocks

                        // Extract rotation and translation for new camera
                        cv::Matx33d R0, R1;
                        cv::Rodrigues(cv::Mat(cv::Vec3d(cam0_data[0], cam0_data[1], cam0_data[2])) * cv::norm(cv::Vec3d(cam0_data[0], cam0_data[1], cam0_data[2])), R0);
                        cv::Rodrigues(cv::Mat(cv::Vec3d(cam1_data[0], cam1_data[1], cam1_data[2])) * cv::norm(cv::Vec3d(cam1_data[0], cam1_data[1], cam1_data[2])), R1);
                        cv::Mat_<double> t0(3, 1, &cam0_data[3]), t1(3, 1, &cam1_data[3]);

                        // Construct projection matrices
                        cv::Matx34d proj_mat0(R0(0, 0), R0(0, 1), R0(0, 2), t0(0),
                                              R0(1, 0), R0(1, 1), R0(1, 2), t0(1),
                                              R0(2, 0), R0(2, 1), R0(2, 2), t0(2));
                        cv::Matx34d proj_mat1(R1(0, 0), R1(0, 1), R1(0, 2), t1(0),
                                              R1(1, 0), R1(1, 1), R1(1, 2), t1(1),
                                              R1(2, 0), R1(2, 1), R1(2, 2), t1(2));

                        // Get 2D points
                        points0[0] = cv::Point2d(observations_[2 * cam_observation_[new_cam_pose_idx][pt_idx]], observations_[2 * cam_observation_[new_cam_pose_idx][pt_idx] + 1]);
                        points1[0] = cv::Point2d(observations_[2 * co_iter.second], observations_[2 * co_iter.second + 1]);
                        ...
                        cv::Mat pt3D = hpoints4D.col(0) / hpoints4D.at<double>(3, 0);
        ...
        // Check for reconstruction divergence
        bool reconstruction_diverged = false;
        ...
        for (int i_c = 0; i_c < num_cam_poses_; i_c++)
        {
            if (cam_pose_optim_iter_[i_c])
            {
                double *prev_camera = cameraBlockPtr(i_c);                              // Previous camera parameters
                double *updated_camera = parameters_.data() + i_c * camera_block_size_; // Updated camera parameters

                // Check if camera parameters have changed significantly
                for (int j = 0; j < camera_block_size_; j++)
                {
                    if (fabs(prev_camera[j] - updated_camera[j]) > divergence_threshold)
                    {
                        reconstruction_diverged = true;
                        break;
                    }
                }

                if (reconstruction_diverged)
                {
                    break; // No need to check further if divergence is detected
                }
            }
        }

        // Check for divergence in point positions
        if (!reconstruction_diverged)
        {
            double *prev_pts = parameters_.data() + num_cam_poses_ * camera_block_size_;    // Previous point parameters
            double *updated_pts = parameters_.data() + num_cam_poses_ * camera_block_size_; // Updated point parameters

            for (int i = 0; i < num_points_; i++)
            {
                // Check if point parameters have changed significantly
                for (int j = 0; j < point_block_size_; j++)
                {
                    if (fabs(prev_pts[i * point_block_size_ + j] - updated_pts[i * point_block_size_ + j]) > divergence_threshold)
                    {
                        reconstruction_diverged = true;
                        break;
                    }
        ...
        // Return false if reconstruction has diverged
        if (reconstruction_diverged)
        {
            return false;
        }
        ...
        for (int i_obs = 0; i_obs < num_observations_; i_obs++)
        {
            ...
            if (cam_pose_optim_iter_[cam_pose_index_[i_obs]] > 0 && pts_optim_iter_[point_index_[i_obs]] > 0)
            {
                ...
                for (int i_obs = 0; i_obs < num_observations_; i_obs++)
                {
                    ...
                    if (cam_pose_optim_iter_[cam_pose_index_[i_obs]] > 0 && pts_optim_iter_[point_index_[i_obs]] > 0)
                    {
                        // Get camera and point blocks
                        double *cam_data = cameraBlockPtr(cam_pose_index_[i_obs]);
                        double *pt_data = pointBlockPtr(point_index_[i_obs]);
                        ...
                        problem.AddResidualBlock(cost_function, new ceres::CauchyLoss(2 * max_reproj_err_), cam_data, pt_data);
                    }
                }
                /////////////////////////////////////////////////////////////////////////////////////////
            }
